...
- A canonical SME-oriented syntax (short, readable, close to natural language).
...
The notation uses one declaration per line, keyword-based properties, and `{ }` blocks for nested elements. This keeps models easy to read and review.
...
|---|---|
| RefModel | `ref "<name>" version "<semver>" { <sections> }` |
| UserType | `user <Name> kind <UserKind> [extends <UserTypeName>(, ...)]` |
| ContextType | `context <Name> kind <ContextKind> [contains <ResourceTypeName>(, ...)]` |
| ResourceType | `resource <Name> [media] [extends <ResourceTypeName>(, ...)] { field ... }` |
| Attribute | `field <name>: <PrimitiveType> [required] [list]` |
| ResourceRelation | `relation <Name> from <ResourceType> [<sourceCard>] to <ResourceType> [<targetCard>] [containment] [recursive]` |
| FeedbackType | `feedback-type <Name> kind <FeedbackKind> [scope <FeedbackSubjectScope>] [rating] [recursive] [media] [text] [polarity <FeedbackPolarity>]` |
| FeedbackDefinition | `feedback <Name> type <FeedbackType> author <UserType> [on resource <ResourceType> \| on feedback <FeedbackDefinition>] [parent <FeedbackDefinition>] [verified-context] [verification <VerificationRequirement>] [unique-per-author-target] [policy ...] [rating ...]` |
| FeedbackPolicy | `policy status <FeedbackStatus>` |
| RatingPolicy | `rating min <number> max <number> [step <number>] [scale <RatingScaleKind>]` |
| ValidationRule | `validation <Name> kind <ValidationKind> [severity <RuleSeverity>] [expr "<expression>"] [impl "<id>"] [applies-resource <ResourceType>] [applies-feedback <FeedbackDefinition>] [applies-user <UserType>] invoked-by <AutomationRule>` |
| ModerationPolicy | `moderation <Name> mode <ModerationMode> [trigger <TriggerEvent>] [decision <ModerationDecision>] monitors resource <ResourceType> feedback <FeedbackDefinition> executed-by <UserType> [context <ContextType>]` |
| AuthorizationRule | `authorize <ActionKind> actor <UserType> [context <ContextType>] resource <ResourceType> [feedback <FeedbackDefinition>]` |
| AutomationRule | `automation <Name> trigger <TriggerEvent> on feedback <FeedbackDefinition> [resource <ResourceType>] [context <ContextType>] uses <ValidationRule> { conditions { ... } actions { ... } }` |
| Condition | `condition <Name> field <AttributeName> op <ConditionOperator> value "<value>"` |
| Action | `action <Name> kind <ActionResultKind> [message "<message>"]` |
| VerificationPolicy | `verification <Name> mode <ValidationKind> when <TriggerEvent> verifies <FeedbackDefinition>` |
| SortingPolicy | `sort <Name> criterion <SortCriterion> direction <SortDirection> [applies-resource <ResourceType>] [applies-feedback <FeedbackDefinition>] [context <ContextType>]` |
...
ref "AmazonRef" version "1.0.0" {
	...
	contexts {
		context CatalogContext    kind CATALOG contains Product, Order, OrderItem, CommentReview
		context ModerationContext kind CATALOG contains Product
		context SearchContext     kind CATALOG contains Product
	}
 
	relations {
		relation OrderContainsOrderItem  from Order     [1] to OrderItem     [2]
		relation OrderItemTargetsProduct from OrderItem [2] to Product        [1]
		relation ProductContainsComment  from Product   [1] to CommentReview [2] containment
	}
 
	feedback-types {
		feedback-type ReviewType      kind REVIEW scope RESOURCE_ONLY rating media text polarity NONE
		feedback-type HelpfulVoteType kind VOTE   scope RESOURCE_ONLY polarity UP_DOWN
	}
 
	feedback-definitions {
		feedback ProductReview type ReviewType author Reviewer on resource Product verification OPTIONAL unique-per-author-target
			policy status ENABLED
			rating min 1 max 5 step 1 scale NUMERIC
		feedback HelpfulVoteOnComment type HelpfulVoteType author Buyer on resource CommentReview verification OPTIONAL
			policy status ENABLED
	}
 
	sorting-policies {
		sort ProductListingSort criterion VALUE direction DESC applies-resource Product        context CatalogContext
		sort ProductSearchSort  criterion DATE  direction DESC applies-resource Product
		sort CommentSortByDate  criterion DATE  direction DESC applies-resource CommentReview  context CatalogContext
		sort CommentSortByRating criterion DATE direction DESC applies-resource CommentReview  context CatalogContext
	}
 
	validations {
		validation ReviewValidationRule kind AUTOMATIC severity ERROR expr "rating_in_range_and_non_empty_text" impl "amazon_review_validation" applies-resource Product applies-feedback ProductReview applies-user Reviewer invoked-by ReviewGuardAutomation
	}
 
	automations {
		automation ReviewGuardAutomation trigger ON_FEEDBACK_CREATE on feedback ProductReview resource Product context ModerationContext uses ReviewValidationRule {
			conditions {
				condition LOl field Description op CONTAINS_KEYWORDS value "g"
			}
			actions {
				action FlagReview kind FLAG_CONTENT message "Content Restricted"
			}
		}
	}
 
	moderations {
		moderation ReviewModerationPolicy mode HYBRID trigger ON_REPORT_THRESHOLD decision FLAGGED monitors resource Product feedback ProductReview executed-by Moderator context ModerationContext
	}
 
	authorizations {
		authorize CREATE   actor Reviewer  context CatalogContext    resource Product       feedback ProductReview
		authorize MODERATE actor Moderator context ModerationContext resource Product       feedback ProductReview
		authorize VOTE     actor Buyer     context CatalogContext    resource CommentReview feedback HelpfulVoteOnComment
	}
 
	verifications {
		verification ProductReviewVerificationAuto   mode AUTOMATIC when ON_FEEDBACK_CREATE verifies ProductReview
		verification ProductReviewVerificationManual mode MANUAL    when ON_MANUAL_REQUEST  verifies ProductReview
	}
}
```
...
1. Domain-friendly keywords replace long class names where possible. For example, `AuthorizationRule` is represented as `authorize`. This improves readability for SMEs.
