/*
 * Copy into pt.isep.enorm.refdsl.v2 project as generator/RefDslGenerator.xtend
 * Uses EMF types from pt.isep.enorm.ref (RefModel from ref.ecore), not refdsl.refDsl.
 * Generates only projections/<name>.txt and .puml (no _structure / _feedback).
 */
package pt.isep.enorm.refdsl.v2.generator
...
- «a.name» trigger=«a.trigger» context=«a.context?.name» inContext=«a.inContext?.name» onFeedback=«a.onFeedback?.name» uses=«a.uses?.name»
«IF !a.conditions.empty»  conditions: «FOR c : a.conditions»[«c.name» «c.field» «c.operator» «c.value»] «ENDFOR»«ENDIF»
«IF !a.actions.empty»  actions: «FOR ac : a.actions»[«ac.name» «ac.kind» «ac.message»] «ENDFOR»«ENDIF»
«ENDFOR»
...
class AutomationRule_«safeId(a.name)» {
  name = «a.name»
  trigger = «a.trigger»
  conditions = «a.conditions.map[it.name + ':' + it.field + ' ' + it.operator + ' ' + it.value].join('; ')»
  actions = «a.actions.map[it.name + ':' + it.kind + ' ' + it.message].join('; ')»
}
