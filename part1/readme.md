...
|---|---|---|---|
| Separate roles (`Buyer`, `Seller`, `Moderator`) | Amazon separates customers who purchase, selling partners who list products, and trust & safety / moderation actors. Distinct roles keep **authorization** and **moderation** responsibilities explicit. | [Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G3UA5WC5S5UUKB5G) (who may contribute reviews and how abuse is handled); [Conditions of Use](https://www.amazon.com/gp/help/customer/display.html?nodeId=508088) | Map each role to `UserType` / `AuthorizationRule` (e.g. who may create feedback vs. who may moderate). |
| `Product` as the main **resource** under a catalog | Shoppers evaluate **items** sold on Amazon; the product detail page is the anchor for ratings, reviews, and Q&A-style interactions. | [Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G3UA5WC5S5UUKB5G); [Understanding Customer Reviews and Ratings](https://www.amazon.com/gp/help/customer/display.html?nodeId=G75XTB7MBMBTXP6W) | Show that feedback in the domain model attaches to **products**, not only to orders. |
| `Order` (or purchase context) linked to the buyer | Reviews and the **Verified Purchase** badge depend on whether the customer bought or used the item through Amazon under eligible conditions. | [Understanding Customer Reviews and Ratings](https://www.amazon.com/gp/help/customer/display.html?nodeId=G75XTB7MBMBTXP6W) (Verified Purchase eligibility); [Your Orders](https://www.amazon.com/gp/css/order-history) | Relate `Order` / purchase evidence to **verification** rules (e.g. `requiresVerifiedContext`, `VerificationPolicy`). |
| Star **review** + text (`REVIEW` / rating policy) | Amazon displays **star ratings** and written **customer reviews** on product pages; ratings aggregate into an overall score. | [Understanding Customer Reviews and Ratings](https://www.amazon.com/gp/help/customer/display.html?nodeId=G75XTB7MBMBTXP6W); [Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G3UA5WC5S5UUKB5G) | Tie `FeedbackType` / `RatingPolicy` (min/max, scale) to how Amazon presents stars and review text. |
| **Verified purchase** and eligibility rules | Amazon labels reviews when purchase/use criteria are met and applies **eligibility** rules (e.g. spending thresholds may apply to contribute ratings/reviews). | [Understanding Customer Reviews and Ratings](https://www.amazon.com/gp/help/customer/display.html?nodeId=G75XTB7MBMBTXP6W) | Map “verified” logic to `VerificationPolicy` / `FeedbackDefinition.requiresVerifiedContext` in your REF instance. |
| **Anti-manipulation** and policy enforcement | Amazon prohibits misleading reviews and manipulation; enforcement blends automated and human measures. | [Anti-Manipulation Policy for Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G8CXDFT9GLRRSV3G); [Trustworthy Shopping — reviews](https://trustworthyshopping.aboutamazon.com/focus/trustworthy-reviews) | Connect to `ValidationRule` / `ModerationPolicy` and automation-style **integrity** checks. |
| **Nested feedback on reviews** (if modelled) | Amazon’s primary published policy focuses on **customer reviews** (stars + text) and eligibility; **follow-up discussion** on a review is a lighter pattern than Reddit threading. Model recursion only if your scenario requires it, and cite the same review policies. | [Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G3UA5WC5S5UUKB5G); [Understanding Customer Reviews and Ratings](https://www.amazon.com/gp/help/customer/display.html?nodeId=G75XTB7MBMBTXP6W) | State explicitly whether your instance models **only product reviews** or also **replies** to reviews, and map to `FeedbackType.recursive` / `parent`. |
| `AuthorizationRule` for who may **read** vs **rate** vs **moderate** | Purchase, community, and seller policies define who can post reviews, edit content, or escalate abuse. | [Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G3UA5WC5S5UUKB5G); [Conditions of Use](https://www.amazon.com/gp/help/customer/display.html?nodeId=508088) | Align `ActionKind` (e.g. READ, RATE, MODERATE) with Amazon’s described permissions and restrictions. |
| `ModerationPolicy` / removal of reviews | Amazon **removes** reviews that violate policies and may take account-level action. | [Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G3UA5WC5S5UUKB5G); [Anti-Manipulation Policy for Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G8CXDFT9GLRRSV3G) | Map moderation outcomes (e.g. hidden/removed) to your `ModerationDecision` / policy enums. |
| `ContextType` as **catalog / marketplace** scope | Product discovery and eligibility often depend on **marketplace context** (same item, same policy family). | [Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G3UA5WC5S5UUKB5G) | Use `ContextKind` (e.g. `CATALOG`) to group resources and rules that share Amazon listing semantics. |
| `AutomationRule` (e.g. triggers on new review) | Amazon uses **automated systems** to detect policy violations and suspicious patterns at scale. | [Anti-Manipulation Policy for Customer Reviews](https://www.amazon.com/gp/help/customer/display.html?nodeId=G8CXDFT9GLRRSV3G) | Position automation as **policy enforcement** and fraud detection, not arbitrary scripting. |

### Reddit

| Domain decision in the model | Why this was modelled this way | Evidence/source (official)                                                                                                      | What to verify/cite in the presentation |
|---|---|---------------------------------------------------------------------------------------------------------------------------------|---|
| Separate user roles (`User`, `Moderator`) | Reddit has distinct user types with different responsibilities. Moderators have governance powers that regular users lack, so role separation is essential. | [Moderator guidelines](https://www.reddithelp.com/en/categories/reddit-101/moderators);                                         | Confirm moderator capabilities (rule enforcement, content removal, user management) and map to `AuthorizationRule`. |
| `Subreddit` as an aggregate that contains posts and community rules | Subreddits are the organizational unit in Reddit. Each community has its own rules, policies, and moderation team. | [Subreddit design](https://reddit.fandom.com/wiki/Subreddit)                                                                    | Cite that subreddits are the primary container for content and governance policies. |
| `Content` superclass with `Post` and `Comment` | Both posts and comments are subject to voting, reporting, and moderation in Reddit. A shared parent enables unified feedback handling. | [Report content](https://support.reddithelp.com/hc/en-us/categories/360003247491-Rules-Reporting)                                         | Justify why posts and comments share common feedback mechanisms (votes, reports). |
...
| `Vote` with upvote/downvote semantics | Reddit's voting system uses upvotes and downvotes to rank content and express community sentiment. This is fundamental to content discovery. | [Upvote/downvote system](https://support.reddithelp.com/hc/en-us/articles/7419626610708-What-are-upvotes-and-downvotes); [Karma](https://support.reddithelp.com/hc/en-us/articles/204511829-What-is-karma)                  | Cite voting as the primary feedback mechanism and its role in ranking and karma calculation. |
| `Report` with status tracking (PENDING, REVIEWED, DISMISSED) | Reporting is a core moderation workflow in Reddit where community members flag content and moderators review. Status tracking reflects the review process. | [Report content](https://support.reddithelp.com/hc/en-us/categories/360003247491-Rules-Reporting); [Moderation tools](https://support.reddithelp.com/hc/en-us/articles/15484384020756-Moderation-Tools-overview) | Map report lifecycle to moderation queue and moderator actions. |
| `AutomationRule` with trigger and condition | Reddit subreddit moderators can set up automations (e.g. auto-remove low karma posts). This enables policy-driven moderation at scale. | [AutoModerator](https://support.reddithelp.com/hc/en-us/articles/15484574206484-Automoderator) | Cite AutoModerator or similar tools; show trigger-condition-action pattern. |
| `CommunityRule` as explicit policy documents | Each subreddit defines rules that govern member behavior. These must be explicit, discoverable, and enforceable. | [Community rules](https://support.reddithelp.com/hc/en-us/articles/15484500104212-Rules) | Verify that community rules align with subreddit policies and are displayed to users. |
| `ParticipationPolicy` controlling POST, COMMENT, VOTE, REPORT actions | Reddit allows subreddit moderators to restrict participation (e.g. member-only posting, karma requirements for commenting). Policies make these controls explicit. | [Karma restrictions](https://reddit.fandom.com/wiki/Karma) | Cite specific participation restrictions (new user posting delay, karma thresholds). |
| `SortingPolicy` determining content order (DATE, VALUE, RELEVANCE) | Reddit provides multiple sorting options (new, hot, top, best, controversial). This is essential for content discovery and user experience. | [Sorting](https://support.reddithelp.com/hc/en-us/articles/19695706914196-What-filters-and-sorts-are-available) | Explain sorting algorithms and their role in content presentation. |
| `ContentValidationRule` targeting POST or COMMENT with severity | Reddit uses automated systems to detect and handle spam, offensive content, and rule violations. Separate rules per content type reflect different moderation signals. | [Community Guidelines](https://www.reddithelp.com/en/categories/reddit-101/rules-reporting); [Anti-spam measures](https://support.reddithelp.com/hc/en-us/articles/360043504051-Spam) | Map content validation rules to specific policy violations (spam, harassment, etc.). |
| `PostModerationCheck` and `CommentModerationCheck` as distinct entities | Post and comment moderation may involve different checks (e.g. link verification, comment toxicity detection). Separate checks reflect these differences. | [Moderation tools](hhttps://support.reddithelp.com/hc/en-us/articles/15484384020756-Moderation-Tools-overview); [Content removal](https://www.reddithelp.com/en/categories/reddit-101/rules-reporting) | Cite moderation workflows and tools specific to post vs. comment moderation. |
| `MediaAttachment` in comments | Reddit comments can include embedded images, GIFs, videos, and links. Media is a first-class element of comment content. | [Post media support](https://support.reddithelp.com/hc/en-us/articles/360043033952-Formatting-Guide) | Verify media types supported and their handling in moderation. |
