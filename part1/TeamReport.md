...
| Member            | Primary tool (individual implementation)                                                              |
|-------------------|-------------------------------------------------------------------------------------------------------|
| Guilherme Cunha   | **JetBrains MPS** - language workbench / projectional editor (`part1/tool1-mps/`, `part1/languages/`) |
| Pedro Vilarinho   | **Eclipse Xtext** - textual DSL, validation, generator (`part1/tool2-xtext/`)                         |
| Francisco Peixoto | **Eclipse Sirius** - graphical views / Viewpoint Specification (`part1/tool3-sirius/`)                |

The team shares a **single domain metamodel (REF)** and three **reference scenarios** (YouTube, Amazon, Reddit). Each member implements the scenario in their assigned tool; **this document** consolidates the **team** work: tool comparison, metamodel rationale, constraints, projection design, and scenario-level application models.
...
The team studied three representative **language workbenches / DSL ecosystems** and recorded a short comparative rationale to justify the split of responsibilities.
...
This mirrors the assignment’s intent: one domain, **multiple notations**, complementary strengths.

---

## 3. Activity 2 - Installation and setup (team summary)

**Assignment note:** Environment setup is primarily an **individual** deliverable (each member documents installation for their tool). At **team** level we record:
...
**Conclusion:** Each member completed tool-specific installation; **this team report** does not duplicate full install logs—refer to the **individual reports** and the files above.

---

## 4. Activity 3 - Domain knowledge (team bridge)

**Assignment note:** Deep scenario analysis is **individual** (YouTube vs Amazon vs Reddit). The **team** consolidates the **union** of domain concepts that motivated the **REF** metamodel.
...
The team captured **why** modelling choices match **public platform documentation** in structured tables (official help / policy links) in **[`part1/readme.md`](readme.md)**—see the **YouTube**, **Amazon**, and **Reddit** sections. Those tables support presentation and traceability from **domain facts** to **metamodel elements** (e.g. mapping moderation and reporting to feedback and policy concepts).

### 4.3 From three products to one metamodel

Instead of three unrelated models, REF is a **generic** metamodel whose **instances** describe each platform: shared notions include **resources**, **user roles**, **feedback**, **validation**, **authorization**, and **automation**. The narrative in [`readme.md`](readme.md) (“Design of the Metamodel”) is the team’s consolidated statement of this generalization.
...
- **`RefModel`** — root container: **user types**, **resource types**, **feedback** definitions, **authorization**, **validation**, **moderation/rating/verification** policies where applicable, **context** types, and **automation** rules.  
...
- **Policies** — moderation, rating, verification, feedback policies as first-class elements where the domain requires distinct treatment across scenarios.
...
Key decisions include: **generalization** over three platforms; **explicit** feedback definitions binding writers, subjects, and rules; **separation** of feedback *type* vs *definition*; **policy** objects for moderation/rating/verification to capture YouTube/Amazon/Reddit differences without forking the core metamodel. Details and examples are expanded in [`part1/readme.md`](readme.md) and in the diagram [`diagrams/metamodel/ref-metamodel-v2.puml`](../diagrams/metamodel/ref-metamodel-v2.puml).
...
| **JetBrains MPS** | **Projectional** editing: the user edits through **editor definitions** bound to the language **structure**; there is no separate “parse” step—the view is always consistent with the AST. | REF language under [`part1/languages/Ref/`](languages/Ref/): **structure**, **editor**, **constraints**, **behavior**, **generator**; sandbox solutions. |
